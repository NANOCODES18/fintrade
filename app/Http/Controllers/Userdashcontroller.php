...
if ( $card->save()) {
    ...
    function dashb_referrals_inactive()
    {
        ...
        $all_inactive_ref = DB::table('referrals')->where('olduseruserid', $this->logged_in_user()->id)->join('users', 'referrals.newuserid', '=', 'users.id')->join('funds', 'referrals.newuserid', '=', 'funds.userid')->where('totalbalance', '<', 0)->get();
